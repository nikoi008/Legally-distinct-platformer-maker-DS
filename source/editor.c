...
{ // causes huge lag when rectangles are large.. todo optimise??
    touchRead(
        &ctx->input
             ->touchPos); // might just be emu lag??? desmume struggles but melonds is fine????? todo test on real ds
    ...
    updateTiles(ctx);
    int highestX =
        (ctx->input->touchTileX > ctx->editor->firstTouchX) ? ctx->input->touchTileX : ctx->editor->firstTouchX;
    int lowestX =
        (ctx->input->touchTileX < ctx->editor->firstTouchX) ? ctx->input->touchTileX : ctx->editor->firstTouchX;
    int highestY =
        (ctx->input->touchTileY > ctx->editor->firstTouchY) ? ctx->input->touchTileY : ctx->editor->firstTouchY;
    int lowestY =
        (ctx->input->touchTileY < ctx->editor->firstTouchY) ? ctx->input->touchTileY : ctx->editor->firstTouchY;
    for(int i = lowestY; i <= highestY; i++)
    {
        for(int j = lowestX; j <= highestX; j++)
        {
            ...
            NF_SetTileOfMap(1,
                            ...
                            screenX,
                            screenY,
                            blocks[ctx->editor->currentBlock].topLeftTile + TRANSPARENT_BLOCK_OFFSET);
            NF_SetTileOfMap(1,
                            TILE_LAYER,
                            screenX + 1,
                            screenY,
                            blocks[ctx->editor->currentBlock].topRightTile + TRANSPARENT_BLOCK_OFFSET);
            NF_SetTileOfMap(1,
                            TILE_LAYER,
                            screenX,
                            screenY + 1,
                            blocks[ctx->editor->currentBlock].bottomLeftTile + TRANSPARENT_BLOCK_OFFSET);
            NF_SetTileOfMap(1,
                            TILE_LAYER,
                            screenX + 1,
                            screenY + 1,
                            blocks[ctx->editor->currentBlock].bottomRightTile + TRANSPARENT_BLOCK_OFFSET);
        }
    }
}
...
void editorFrame(gameContext *ctx)
{
    ...
    if(!ctx->input->keyboardOn)
    {
        ...
        {
            if(ctx->input->touchPos.py >= HUD_Y_START && ctx->input->touchPos.py <= 192 - 22)
            { //hud width
        ...
        {
            if(ctx->input->touchPos.py <= HUD_Y_START)
            { //hud width
                ...
                else
                {
                    ...
                    if(touchBlock < TOTAL_BLOCKS)
                    {
                        ctx->editor->currentBlock = touchBlock;
                    }
                }
            }
            updateTiles(ctx);
        }
        else if(ctx->editor->rectFillOn && (ctx->editor->firstTouchX < 0 || ctx->editor->firstTouchY < 0) &&
                ctx->input->buttonsHeld & KEY_TOUCH && !touchedToggleButton)
        {
                ...
                ctx->input->buttonsHeld & KEY_TOUCH && !touchedToggleButton)
        {
            rectangleFillPreview(ctx);
                ...
                ctx->input->buttonsUp & KEY_TOUCH && !touchedToggleButton)
        {
            fill(ctx);
        }
